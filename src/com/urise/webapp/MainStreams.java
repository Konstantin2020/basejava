...
// вызов oddOrEven через через Streams
        System.out.println("Результат: " + oddOrEven(new ArrayList<>(Arrays.asList(1, 2, 2, 1, 1))));
        System.out.println(" ");
...
// реализация minValue через Streams
    private static int minValue(int[] values) {
        ...
        System.out.println(" ");
        return IntStream.range(0, Arrays.stream(values).distinct().sorted().toArray().length)
                .map(i -> (int) (Math.pow(10, Arrays.stream(values).distinct().sorted().toArray().length - 1 - i) * Arrays.stream(values).distinct().sorted().toArray()[i]))
                .sum();
    }
...
// реализация oddOrEven через через Streams
    private static List<Integer> oddOrEven(List<Integer> integers) {
        ...
        System.out.println("Сумма элементов: " + integers.stream().mapToInt(a -> a).sum());
        return integers.stream().flatMap(y -> {
